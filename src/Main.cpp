...
int main(int /*argc*/, char **/*argv*/) {
     internal = new cluon::OD4Session(111, [](cluon::data::Envelope &&envelope) noexcept {
        if (envelope.dataType() == opendlv::proxy::GroundSteeringReading::ID()) {
            ...
            std::cout << "GroundSteering_msg sent! " << receivedMsg.groundSteering() << std::endl;
        }
            ...
            std::cout << "PedalPosition_msg sent!" << receivedMsg.position() << std::endl;
        }
    });
    ...
    external = new cluon::OD4Session(112, [](cluon::data::Envelope &&envelope) noexcept {
        if (envelope.dataType() == 2001) {
            ...
            msgSteering.groundSteering(ins.steeringAngle() -45);
            msgPedal.position(ins.pedalPosition() /100);
    ...
    while (1) {
        ...
        if (now.time_since_epoch().count() -last_instruction_timestamp > 500) {
            SteeringInstruction brake;
            brake.pedalPosition(0);
            brake.steeringAngle(45);
            external->send(brake);
        }
